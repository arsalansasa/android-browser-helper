...
public class TwaProviderPicker {
    ...
    public static Action pickProvider(PackageManager pm) {
        List<String> chromePackages = new ArrayList<String>(Arrays.asList("com.android.chrome","com.chrome.beta","com.chrome.dev","com.google.android.apps.chrome"));
            ...
            switch (launchMode) {
                case LaunchMode.TRUSTED_WEB_ACTIVITY:
                    for (String chromePackage : chromePackages){
                        if(providerName.equals(chromePackage)){
...
//                    return new Action(LaunchMode.TRUSTED_WEB_ACTIVITY, providerName);
                case LaunchMode.CUSTOM_TAB:
                    for (String chromePackage : chromePackages){
                        if(providerName.equals(chromePackage)){
                            Log.d(TAG, "Found Chrome as Custom Tabs provider: " + providerName);
...
//                    break;
                case LaunchMode.BROWSER:
                    ...
                    for (String chromePackage : chromePackages){
                        if(providerName.equals(chromePackage)){
                            Log.d(TAG, "Found Chrome as Regular provider: " + providerName);
...
//                    if (bestBrowserProvider == null) bestBrowserProvider = providerName;
        ...
        Log.d(TAG, "Didn't found chrome as provider ");
        if(possibleProviders.isEmpty()){
            Log.d(TAG, "No providers found");
            return new Action(LaunchMode.TRUSTED_WEB_ACTIVITY, "");
        } else {
            Log.d(TAG, "Using another provider : " + possibleProviders.get(0).activityInfo.packageName);
            return new Action(LaunchMode.TRUSTED_WEB_ACTIVITY, possibleProviders.get(0).activityInfo.packageName);
        }
    }
